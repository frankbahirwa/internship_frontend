            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-foreground">
                        {t('dashboard.manage_hospitals').split(' ')[0]} <span className="text-ruby">{t('dashboard.hospitals')}</span>
                    </h1>
                    <p className="text-foreground/50 font-medium mt-1">
                        {t('dashboard.manage_hospitals_desc')}
                    </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex items-center gap-2 bg-obsidian-light rounded-xl px-4 py-2 border border-foreground/5">
                        ...
                        <input
                            type="text"
                            placeholder={t('dashboard.search_facilities')}
                ...
                <AnimatePresence>
                    {filteredHospitals.length > 0 ? (
                                ...
                                <div className="flex items-center gap-4 flex-1">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <StatusBadge status={hospital.verificationStatus} t={t} />
                                        </div>
                                        <div className="flex flex-wrap items-center gap-4 text-xs text-foreground/40">
                                            <span className="flex items-center gap-1">
                                                <ShieldCheck className="w-3 h-3" /> {t('dashboard.license_id')}: {hospital.licenseNumber}
                                            </span>
                                            <span className="flex items-center gap-1">
                                ...
                                <div className="flex items-center gap-3 w-full md:w-auto">
                                    {hospital.verificationStatus === 'PENDING' && (
                                        <>
                                            ...
                                            >
                                                <CheckCircle2 className="w-3 h-3" /> {t('dashboard.verify')}
                                            </button>
                                            <button
                                            ...
                                            >
                                                <XCircle className="w-3 h-3" /> {t('dashboard.reject')}
                                            </button>
                                        </>
                                    )}
                                    <button
                                    ...
                                    >
                                        {hospital.user?.isActive ? t('dashboard.active') : t('dashboard.banned')}
                                    </button>
                                </div>
                            </motion.div>
                        ))
                        ...
                        <div className="py-20 text-center glass rounded-3xl border-dashed border-2 border-foreground/10">
                            <p className="text-foreground/40 font-medium">{t('dashboard.no_hospitals_found')}</p>
                        </div>
                    )}
                </AnimatePresence>
            </div>
